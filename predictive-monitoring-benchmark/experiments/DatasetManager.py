...
class DatasetManager:
    ...
    def add_ngram(self, group, ngr):
        group = group.sort_values(self.timestamp_col, ascending=True, kind='mergesort').reset_index(drop=True)
        results = pd.DataFrame()
        ngram_value = ngr
        for index, row in group.iterrows():
            df = pd.DataFrame(group.iloc[index:index + ngram_value, :], columns=['Case ID', 'Activity', 'time:timestamp', 'SLA MIN', 'dur_min', 'label',
       'wip'])
    ...
    # GAP parameter
    def generate_prefix_data_ngram(self, data, ngram_s):  # min_length, max_length, gap=1):
        ngr = ngram_s
        dt_prefixes = data.groupby('Case ID').apply(self.add_ngram, ngr)
    ...
    def get_prefix_lengths(self, data):
        ...
        return data.groupby(self.case_id_col).last()["subsequence_nr"]
